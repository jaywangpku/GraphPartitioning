...
def NEPartition(edgesFile, numOfPars):
    ...
    for line in f:
        ...
        # inOutDeg {verID : (inD, outD)} 维护每个点的 出入度信息
        ...
        #vertexLinks, (inVertices, outVertices)  维护每个点的 出入对应的链接信息
    ...
    # to construct a list of blocks' size           construct  构造
    blockSizes = []
    ...
    for sd, bsize in sortedBS:
        ...
        min_size += bsize
        for i in range(0, len(mergingRs)):            # 选择最小的 part 然后往里面发分好的块
    ...
    # a list to store nodes and indicator of seed     indicator  指示符
    nodes = []
    for v in vertices:
        if v in seeds:
            nodes.append((v, "s"))
