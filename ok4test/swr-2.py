...
edgeNum = 0
# 窗口初始大小
windowsize = 100000  # 30622564  68993773
# 最大窗口大小
...
while(line < len(edges)):
    ...
    for i in range(len(window)):
        src = window[i][0]
        tar = window[i][1]
        if(ver2degreeDic[src] == 1):
            windowSort.append((src, tar))
        elif(ver2degreeDic[tar] == 1):
            windowSort.append((tar, src))
        elif(ver2degreeDic[src] < ver2degreeDic[tar]):
            windowSort.append((src, tar))
        else:
            windowSort.append((tar, src))
    window = sorted(list(windowSort), key=lambda x: (ver2degreeDic[x[0]]))
    cut1 = int(len(window) * 0.35)
    windowRand1 = window[0 : cut1]
    window = window[cut1 : len(window)]
    ...
    for i in range(len(window)):
        ...
        if(ver2degreeDic[src] < ver2degreeDic[tar]):
            ...
            windowSort.append((tar, src))
    window = sorted(list(windowSort), key=lambda x: (ver2degreeDic[x[0]]))
    cut2 = int(len(window) * 0.5)
    windowRand2 = window[0 : cut2]
