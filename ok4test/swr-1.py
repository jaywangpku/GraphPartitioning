...
while(line < len(edges)):
    while(len(window) < windowsize and line < len(edges)):   # 将窗口填满，或者是最后一点了
        ...
        if(ver2degreeDic.has_key(tar)):
            ...
            ver2degreeDic[tar] = 1
        # windowVertices.add(src)
        # windowVertices.add(tar)
    ...
    # windowVertices = sorted(list(windowVertices), key=lambda x: (ver2degreeDic[x]))
    # for i in range(len(windowVertices)):
    #     print windowVertices[i]
    #     print ver2degreeDic[windowVertices[i]]
    #     print "\n"
    ...
    # cut2 = cut1 + int(okflag*(len(window) -  cut1))
    ...
    # windowTemp = []
    # for i in range(rand):
    #     start = i * len(window) / rand
    #     end = (i+1) * len(window) / rand
    #     windowTemp = window[start : end]
    #     random.shuffle(windowTemp)
    #     for i in range(len(windowTemp)):
    #         windowRand.append(windowTemp[i])
    # window = windowRand

    # cut1 = int(len(window) * 0.3)
    # cut2 = int(len(window) * 0.6)
    # windowRand1 = window[0 : cut1]
    # windowRand2 = window[cut1 : cut2]
    # windowRand3 = window[cut2 : len(window)]
    # random.shuffle(windowRand1)
    # random.shuffle(windowRand2)
    # random.shuffle(windowRand3)
    # window = windowRand1 + windowRand2 + windowRand3

    # random.shuffle(window)  # 现在不采取打乱这个策略
